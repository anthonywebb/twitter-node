...
TwitterNode.prototype.track = function track(word) {
  if (-1 == this.trackKeywords.indexOf(word)) {
    this.trackKeywords.push(word);
  }
  return this;
};

TwitterNode.prototype.stopTracking = function stopTracking(word) {
  var idx = this.trackKeywords.indexOf(word);
  if (-1 != idx) {
    this.trackKeywords.splice(idx, 1);
  }
  return this;
};

TwitterNode.prototype.clearTracking = function clearTracking() {
  this.trackKeywords = [];
  return this;
};
...
TwitterNode.prototype.stream = function stream() {
  if (this._clientResponse && this._clientResponse.connection) {
    this._clientResponse.socket.destroy();
  }
...
TwitterNode.prototype.stop = function stop() {
  if (this._clientResponse && this._clientResponse.connection) {
    this._clientResponse.socket.destroy();
    delete this._clientResponse;
  }
  return this;
}
